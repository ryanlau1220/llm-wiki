...
export interface CommunityInfo {
  id: number;
  nodeCount: number;
  cohesion: number; // internal connections / possible connections
  topNodes: string[]; // labels of top nodes in community
}
...
export function detectKnowledgeGaps(
  ...
  if (isolatedNodes.length > 0) {
    ...
    gaps.push({
      ...
      description: topIsolated.map((n) => n.title || n.id).join(", ") +
