    ...
    <>
      ...
      >
        <div className="p-4 flex items-center justify-between border-b border-[var(--line)]">
          {!isCollapsed && (
            <span className="font-bold text-lg text-[var(--sea-ink)]">LLM Wiki</span>
          )}
          <button
            type="button"
            onClick={() =>
            ...
            {navItems.map((item) => (
              <li key={item.label}>
                <Link
                  ...
                  activeProps={{
                    className: "bg-foam/80 border-line text-sea-ink font-semibold shadow-sm",
                  }}
                  inactiveProps={{
                    className: "text-sea-ink-soft hover:bg-foam/50 border-transparent",
                  }}
